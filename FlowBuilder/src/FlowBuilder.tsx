...
export default function FlowBuilder(props: FlowBuilderProps) {
  ...
  const generateWorkflowJson = (
    ...
    workflow.push({
      ...
      "username": "7e1btjduhd5b174",
      "password": "efdb9h89bdkw2dc",
      "secure": false,
      "tls": false,
      "name": "",
      "dname": "",
    ...
    workflow.push({
      ...
      "func": "msg.params = [msg.payload.requestid, msg.payload.workflowName, msg.payload.status, msg.payload.remark];\nnode.warn(msg);\nreturn msg;\n",
      "outputs": 1,
      "timeout": 0,
      "noerr": 0,
      "initialize": "",
    ...
    workflow.push({
      "id": "approver_email",
        ...
        "z": tabId,
        "name": "approver_email",
        "func": "\nnode.warn(msg);\nlet status = \"Pending\";\n// Prepare email content\nmsg.request_id = msg.payload.requestid; // Use the dynamic requestId\nmsg.topic = \"Workflow \" + msg.request_id; // Use string concatenation instead of template literals\nmsg.to = msg.payload.approver_email[0]; // || \"herig68683@cybtric.com\";\n\nmsg.html = '<div style=\"font-family: Arial, sans-serif; padding: 15px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;\">' +\n  '<h2 style=\"color: #2c3e50;\">Workflow Request Update</h2>' +\n  '<p style=\"font-size: 16px;\">Workflow request is Pending for the Employee ID ' + msg.request_id + '. Please click the <a href=\"http://ec2-52-91-38-126.compute-1.amazonaws.com:9000\">Link</a> to approve or reject</p>' +\n  '<table style=\"width: 100%; border-collapse: collapse; margin-top: 10px;\">' +\n      '<tr><td style=\"padding: 10px; border: 1px solid #ddd; background-color: #ecf0f1;\"><strong>Request ID:</strong></td>' +\n      '<td style=\"padding: 10px; border: 1px solid #ddd;\">' + msg.request_id + '</td></tr>' +\n      '<tr><td style=\"padding: 10px; border: 1px solid #ddd; background-color: #ecf0f1;\"><strong>Status:</strong></td>' +\n  '<td style=\"padding: 10px; border: 1px solid #ddd; color: ' + (status === 'Completed' ? 'green' : 'red') + ';\">' +\n  '<strong>' + status + '</strong></td></tr></table>' +\n      '<p style=\"margin-top: 15px; font-size: 14px; color: #7f8c8d;\">This is an automated message. Please do not reply.</p>' +\n      '</div>';\n\nmsg.payload = msg.html;\n\nreturn msg;",
        "outputs": 1,
        "timeout": 0,
        "noerr": 0,
        "initialize": "",
